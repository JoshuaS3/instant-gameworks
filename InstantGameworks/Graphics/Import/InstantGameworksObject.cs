...
namespace InstantGameworks.Graphics.Import
{
    ...
    public class DataHandler
    {
        ...
        {
            using (Stream _fileStream = new FileStream(fileLocation, FileMode.Create, FileAccess.Write, FileShare.None))
            {
        ...
        public static InstantGameworksObject ReadFile(string fileLocation)
        {
            ...
            using (Stream _fileStream = new FileStream(fileLocation, FileMode.Open, FileAccess.Read, FileShare.Read))
            {
                using (BinaryReader _binaryReader = new BinaryReader(_fileStream))
                {
                    ...
                    for (int i = 0; i < arrayLength; i++)
                    {
                        ...
                        {
                            U = _binaryReader.ReadSingle(),
                            V = _binaryReader.ReadSingle(),
                            W = _binaryReader.ReadSingle()
                        };
                    ...
                    for (int i = 0; i < arrayLength; i++)
                    {
                        ...
                        for (int x = 0; x < 3; x++) //for each vertex
                        {
                            ...
                            {
                                PositionIndex = _binaryReader.ReadInt32(),
                                TextureCoordinatesIndex = _binaryReader.ReadInt32(),
                                NormalIndex = _binaryReader.ReadInt32()
                            };
